...
export default function PortfolioSummaryCard({ portfolio, custodyScore, onCustodyPress }: Props) {
  ...
  return (
    <LinearGradient colors={['#1a1e3a', '#0A0C14']} style={styles.card}>
      <Text style={styles.label}>Total Portfolio</Text>
      <Text style={styles.value}>${portfolio.totalValueUSD.toLocaleString(undefined, { maximumFractionDigits: 0 })}</Text>
...
const styles = StyleSheet.create({
  card: { borderRadius: 20, padding: 24, marginBottom: 8 },
  label: { color: colors.text.secondary, fontSize: 13, fontWeight: '500', marginBottom: 4 },
  value: { color: colors.text.primary, fontSize: 40, fontWeight: '700', letterSpacing: -1, marginBottom: 8 },
  row: { flexDirection: 'row', alignItems: 'center', marginBottom: 16 },
  separator: { color: colors.text.tertiary },
  healthBar: { flexDirection: 'row', alignItems: 'center', gap: 10 },
